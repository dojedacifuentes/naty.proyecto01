...
if (!codigos.length) {
  console.error('Uso: npm run kit -- PF1821 [PF1822 ...] [--salida <dir>] [--pdf]');
  process.exit(1);
}
...
function documento(codigo) {
  const p = JSON.parse(leer(`data/planes/${codigo}.json`));
  const m = p.modulos[1];
  const dir = `contenidos/${codigo}/modulo-2`;
  const partes = ORDEN.filter(([f]) => existe(`${dir}/${f}`));
  const faltan = ORDEN.filter(([f]) => !existe(`${dir}/${f}`)).map(([f]) => f);
  const hoy = new Date().toLocaleDateString('es-CL', { day: 'numeric', month: 'long', year: 'numeric' });
  const titulo = `Kit de recursos del módulo 2 · ${codigo}`;
  return { faltan, html: `<!doctype html>
...
<section class="cover">
  <p class="eb">Becas Laborales · Talento Digital 2026 · ${codigo}</p>
  <h1>Kit de recursos educativos<br>del módulo 2</h1>
  <div class="rule"></div>
  <p class="sub">${codigo} · ${esc(p.nombre)}<br>Módulo 2: ${m.codigo} ${esc(m.nombre)} · ${m.horas} h</p>
  <div class="toc">${partes.map(([, t], k) => `<div><b>${k + 1}</b>${t}</div>`).join('')}</div>
  <div class="meta">
    <b>Para:</b> Natalia — hackea.pro<br>
    <b>Fecha:</b> ${hoy}<br>
    <b>Estado:</b> borrador para revisión
  </div>
  <p class="foot">Generado desde el repositorio del proyecto (npm run kit) a partir de contenidos/${codigo}/modulo-2/. Aprendizajes y competencia textuales de SIPFOR (${esc(p.fuente.resolucion ?? '')}, ${esc(p.fuente.fecha_resolucion ?? '')}). Lo marcado PENDIENTE lo decide cada institución.</p>
</section>
${partes.map(([f, t], k) => `<section class="parte"><p class="kicker">${k + 1} · ${t}</p>${markdown(leer(`${dir}/${f}`))}</section>`).join('\n')}
</body></html>` };
}
...
for (const c of codigos) {
  ...
  const { html, faltan } = documento(c);
  const base = `${salida}/kit-recursos-modulo2-${c}`;
  console.log(`${c}  HTML → ${escribir(`${base}.html`, html)}${faltan.length ? `  (faltan: ${faltan.join(', ')})` : ''}`);
  if (conPdf) {
    try { console.log(`${c}  PDF  → ${base}.pdf (${imprimirPdf(`${base}.html`, `${base}.pdf`)} KB)`); }
